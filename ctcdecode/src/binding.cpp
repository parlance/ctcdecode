...
int beam_decode(at::Tensor th_probs,
    ...
    std::vector<std::vector<std::pair<double, Output>>> batch_results =
            ctc_beam_search_decoder_batch(inputs, new_vocab, beam_size, num_processes, cutoff_prob, cutoff_top_n,
    ...
    for (auto results: batch_results) {
        for (auto n_path_result: results) {
...
void *paddle_get_scorer(double alpha,
                        double beta,
                        const char *lm_path,
                        const vector<std::string> &new_vocab,
                        int vocab_size) {
    auto *scorer = new Scorer(alpha, beta, lm_path, new_vocab);
...
std::pair<torch::Tensor, torch::Tensor> beam_decode_with_given_state(at::Tensor th_probs,
    ...
    for (auto &batch_result: batch_results) {
        std::vector<std::pair<double, Output>> results = batch_result;
        if (batch_result.size() > max_result_size) {
            max_result_size = batch_result.size();
        }
        for (const auto &n_path_result: results) {
            Output output = n_path_result.second;
            std::vector<int> output_tokens = output.tokens;
    ...
    torch::Tensor output_tokens_tensor = torch::randint(1,
                                                        {static_cast<long long>(batch_results.size()), max_result_size,
                                                         max_output_tokens_size});
    torch::Tensor output_timesteps_tensor = torch::randint(1, {static_cast<long long>(batch_results.size()),
                                                               max_result_size, max_output_tokens_size});
...
std::pair<torch::Tensor, torch::Tensor> paddle_beam_decode_with_given_state(at::Tensor th_probs,
    ...
    return beam_decode_with_given_state(th_probs, th_seq_lens, num_processes, states, is_eos_s, th_scores,
                                        th_out_length);
}
...
void *paddle_get_decoder_state(const std::vector<std::string> &vocabulary,
    ...
    auto *state = new DecoderState(vocabulary, beam_size, cutoff_prob, cutoff_top_n, blank_id, log_input, ext_scorer);
...
int is_character_based(void *scorer) {
    auto *ext_scorer = static_cast<Scorer *>(scorer);
