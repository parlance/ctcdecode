...
decoder = CTCBeamDecoder(labels, model_path=None, alpha=0, beta=0, cutoff_top_n=40, cutoff_prob=1.0, beam_width=100,
...
### Inputs to `CTCBeamDecoder`
 ...
 - `model_path` is the path to your language model(LM), if you are using one. CTCDecode works with KenLM
 - `alpha` and `beta` are weights that control how the LM conducts the beam search **more info needed**
 - `cutoff_top_n` I'd imagine this only returns the top n beams, but in my experience it doesn't do anything **more info needed**
 - `cutoff_prob` **more info needed**
 ...
 - `num_processes` Default 4. You probably want to pass the number of cpus your computer has. You can find this in python with `import multiprocessing` then `n_cpus = multiprocessing.cpu_count()`
 ...
 - `log_probs_input` Default True. If your outputs have passed through a softmax and represent probabilities, this should be false, if they passed through a LogSoftmax and represent negative log likelihood, you need to pass True. If you don't understand this, run `print(output[0][0].sum())`, if it's a negative number you've probably got NLL and need to pass True, if it sums to ~1.0 you should pass False.

### Inputs to the `decode` method
 - `output` should be the output of your model. If your output has passed through a normal Softmax, you shouldn't need to alter it (except maybe to transpose), but if your `output` represents negative log likelihoods (raw logits), you either need to pass it through an additional `torch.nn.functional.softmax` or you can pass `log_probs_input=False` to the CTCDecoder. Your output should be BATCHSIZE x N_TIMESTEPS x N_LABELS so you may need to transpose it before passing it to the decoder. Note that if you pass things in the wrong order, the beam search will probably still run, you'll just get back nonsense results. 
...
4 things get returned from `decode`
 ...
 1. `timesteps` - Shape: BATCHSIZE x N_BEAMS The timestep at which the nth output character first appears **more info needed**
