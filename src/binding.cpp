...
beam_decode(
  ...
  auto seq_lens = [&](){
    if (seq_lens_.has_value()) {
      ...
      TORCH_CHECK(
        (seq_lens <= max_seq_len).all().item<bool>(),
        "All the values in`seq_lens` must be less than or equal to the length of `probs`.");
      return seq_lens;
    }   
  ...
  at::parallel_for(0, batch_size, grain_size, [&](int64_t begin, int64_t end) {
      for (auto i = begin; i < end; ++i) {
        batch_results[i] = ctc_beam_search_decoder(inputs[i],
                                                   vocabulary,
                                                   beam_size,
                                                   cutoff_prob,
                                                   cutoff_top_n,
